 ...
 * @param $path
 */
function spider($province, $city, $path)
{
    //数据篮子
    $data = [];
    ...
    foreach ($province as $k => $area) {
        ...
        //创建区县文件夹
        $current_path = $path.'/'.$areaName;
        mkdirs(iconv('utf-8', 'gbk', $path.'/'.$areaName));
        ...
        foreach ($area['area'] as $z => $house) {
            ...
            foreach ($houseData['loupan']['hit'] as $x => $houseDetail) {
                ...
                $data = [
                    ...
                    'type'                           => !empty($type[1]) ? (strpos($type[1],
                        '楼') !== false ? '写字楼' : ((strpos($type[1], '商') !== false || strpos($type[1],
                            '购') !== false) ? '商场' : $type[1])) : '小区',
                    //物业类型
...
//                }
            ...
            $street_name = $house['name'] ?? '无无';
        }
        if(!empty($datas)){
            $current_path = iconv('utf-8', 'gbk', $current_path.'/'.$street_name.'.json');
            file_put_contents($current_path, json_encode($datas, JSON_UNESCAPED_UNICODE));
            $current_path = '';
            echo $current_path.'/'.$street_name.'.json ok';
        }
    }
}
